...
hashable_equatable_values = (
        _st.none() | _st.booleans() | _st.integers()
        | _st.fractions() | _st.floats(allow_nan=False)
        | _st.complex_numbers(allow_nan=False) | _st.binary() | _st.text()
        | _st.dates() | _st.datetimes()
)
...
equatable_values |= _st.dictionaries(hashable_equatable_values,
                                     deferred_equatable_values)
empty_factories = _st.builds(lambda value: (lambda: value), equatable_values)
equatable_pure_maps = (
